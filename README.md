...
- 免费内容：
  - 官方文档明确有“新人免费额度”
  - 部分模型在指定地域下有免费时长或免费 token 配额；官方新人免费额度 FAQ 顶部规则当前写明仅华北 `2`（北京）地域模型享有新人免费额度，其他地域无免费额度；同一 FAQ 和价格表中如出现新加坡地域 / 国际部署模型免费额度，应按具体模型行重新核对
  - 官方新人免费额度说明写明，首次开通百炼时会自动发放符合条件模型的新人免费额度；当前主规则为 `90 天`，`2025-09-08 11:00` 前已开通用户的有效期可能不足 `90 天`
- 使用限制：
  - 免费额度主路径适用于华北 `2`（北京）地域模型实时推理；新加坡地域 / 国际部署模型不要默认视为有新人免费额度，必须按当前模型行确认
...
- 使用限制：
  ...
  - `Coding Plan` 官方当前默认模型标识为 `astron-code-latest`，并列出 `Spark X2` / `Spark X2 Agent`、`GLM-5.2` / `GLM-5.1` / `GLM-5`、`DeepSeek-V4-Pro`、`DeepSeek-V4-Flash`、`DeepSeek-V3.2`、`Kimi-K2.7-Code`、`Kimi-K2.6`、`MiniMax-M2.5`、`Qwen3.5` / `Qwen3.6`、`Qwen3-Coder-Next-FP8`、`GLM-4.7-Flash` 等当前模型池示例；`2026-07-23` 版本的模型抵扣系数表中 `Kimi-K2.7-Code` 的 `modelId` 为 `xopkimi27code`，抵扣系数为 `5`
