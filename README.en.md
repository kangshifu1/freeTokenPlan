...
- DashScope / Bailian: free quota is regional and activity-based. The official new-user free-quota FAQ says first activation automatically issues eligible model quotas, and its top rule currently says only China North `2` (Beijing) models receive new-user free quota while other regions do not. If the same FAQ or pricing tables show Singapore-region / international-deployment free quota for a specific model, verify that exact model row. The current main validity rule is `90 days`; accounts enabled before `2025-09-08 11:00` may have shorter validity.
