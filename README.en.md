...
| [Tencent Hunyuan / TokenHub](providers/tencent-hunyuan/README.en.md) | Legacy Hunyuan shared `1M tokens / 1 year` package; verify new capabilities in TokenHub | Hunyuan text generation / TokenHub models | Yes | [TokenHub](https://console.cloud.tencent.com/tokenhub) |
...
- iFlytek Astron MaaS: the public model square is the discovery entry, but the current coding-tool path is documented through separate `Token Plan` and `Coding Plan` products. Both publish OpenAI- and Anthropic-compatible endpoints, use dedicated plan-specific API keys, and should not be confused with the regular MaaS inference base URL. `Token Plan` now lists refreshed model examples such as `GLM-5.2`, `DeepSeek-V4-Pro`, `DeepSeek-V4-Flash`, `Kimi-K2.6`, and `Qwen3.6`, with point-based usage and a `0.8` off-peak time coefficient. `Coding Plan` still uses `astron-code-latest` as the official default model identifier, adds a Responses-compatible endpoint, lists the same refreshed model families, marks the first-month plan unavailable for purchase from `2026-04-09 00:00`, supports monthly renewed-version purchases from `2026-04-09` and quarterly renewed-version purchases from `2026-06-15`, and applies a `2026-06-18` off-peak coefficient to request limits. Treat it as an interactive coding-tool plan, not a backend batch API.
- Tencent Hunyuan / TokenHub: the current billing overview says Hunyuan capabilities are gradually moving to TokenHub, so new model services or expanded capabilities should be checked in TokenHub instead of assuming the legacy Hunyuan console path. First-time Hunyuan activation after real-name verification can issue a one-time shared `1M tokens` package valid for `1 year` across listed Hunyuan text and multimodal models, excluding `Hunyuan-lite`; free packages are consumed before paid packages and postpaid billing, and exhausted or expired packages do not automatically roll into postpaid unless the user enables postpaid.
