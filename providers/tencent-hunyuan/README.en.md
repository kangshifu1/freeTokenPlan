...
- TokenHub trial package: the official TokenHub new-user trial page currently lists a language-model trial package: new users can claim `1M tokens` after real-name verification, valid for `90 days`, for language models supported by TokenHub
...
- Compatibility: Tencent Hunyuan documents OpenAI-compatible access; current TokenHub / Hunyuan base URLs, authentication details, and model names should be verified in official docs. Do not mix the TokenHub trial package with the legacy Hunyuan package when recording entry point, validity, or model scope
