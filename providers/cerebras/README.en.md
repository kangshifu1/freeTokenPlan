...
- Limits: the Free Trial tier currently lists `5 RPM` / `30K TPM` / `1M TPH` / `1M TPD` for `gpt-oss-120b`, `zai-glm-4.7`, and `gemma-4-31b`; token rate limiting is now documented as a dual-bucket model with independent uncached and total token limits, with uncached-token visibility rolling out to consoles by `2026-08-17`; actual account limits still depend on the console and current docs
