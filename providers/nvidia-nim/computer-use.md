...
3. 打开目标模型页，确认 `Model Availability` 中 `Free Endpoint` 是否仍为 `Available`
4. 点击页面里的 `Get API Key`、`Build with this model` 或类似入口
5. 生成并复制 API key
6. 本地保存为环境变量：`NVIDIA_API_KEY`
7. 记录当前模型 ID、调用端点、`Free Endpoint` 状态和页面显示的速率限制
...
- NVIDIA 账号登录 / 邮箱验证
- 组织或开发者条款确认
- 模型页的地区、速率、`Free Endpoint: Deprecated` 或其他可用性提示

## 完成后核对

- API key 是否创建成功
- 当前模型页是否仍显示 `Free Endpoint: Available`
