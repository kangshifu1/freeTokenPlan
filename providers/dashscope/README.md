...
- 注册：阿里云账号开通百炼
- 免费：首次开通百炼时平台会自动发放各模型新人专属免费额度；官方说明免费额度仅适用于华北 `2`（北京）地域、中国内地部署范围的模型实时推理，不抵扣 Batch、模型调优、模型部署或自定义模型费用
- 有效期：官方当前规则为 `30～90 天`，且 `2025-09-08 11:00` 起首次开通用户获赠的新人免费额度有效期调整为 `90 天`
