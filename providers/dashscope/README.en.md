...
- Free tier: when Bailian is first enabled, Alibaba Cloud automatically issues new-user free quota for each eligible model; current official docs limit this quota to realtime inference for models in China mainland deployment scope and the China North `2` (Beijing) region, excluding Batch calls, fine-tuning, deployments, and custom models
