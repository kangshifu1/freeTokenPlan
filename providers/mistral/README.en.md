...
- Key / billing boundary: API keys are not tied to a billing mode; the same key follows the workspace's current plan and billing state as it moves from `Free mode` to `Pay as you go` or a higher tier
